...
const getListOrder = async (nfOrder) => {
  ...
  fetch(url, {
    ...
    data.order.comentarios.forEach(comentarios => insertComments(comentarios.texto));
    data.order.produtos.forEach(produtos => insertItens(produtos.pedido_nf, produtos.mome, produtos.quantidade, produto.valor, produtos.quantidade, produtos.valor_total));
  }).catch((error) => {
    console.error('Error:', error);
  });
}
