...
produto_tag = Tag(name="Produto", description="Adição, visualização e remoção de produtos à um pedido")
...
def get_produtos_pedido(query: ProdutoBuscaSchema):
    ...
    if not produtos:
        ...
        logger.debug(f"%d rodutos econtrados", (produtos))
        # retorna a representação de produto
        print(produtos)
        return apresenta_produtos(produtos), 200
...
def del_pedido(query: PedidoBuscaSchema):
    ...
    session = Session()
    print (nf)
    logger.debug(f"Deletando dados sobre pedido #{nf}")
...
def add_comentario(form: ComentarioSchema):
    ...
    """
    pedido_nf = form.pedido_nf
    logger.debug(f"Adicionando comentários ao produto #{pedido_nf}")
    # criando conexão com a base
    session = Session()
    ...
    logger.debug(f"Adicionado comentário ao produto #{pedido_nf}")
