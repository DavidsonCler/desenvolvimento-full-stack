...
/*
  ...
  --------------------------------------------------------------------------------------
*/
const postItem = async (inputNf, inputClient, inputAddress, inputMobile) => {
  const formData = new FormData();
  ...
  let url = 'http://127.0.0.1:5000/produto_pedido';
  fetch(url, {
    method: 'post',
    body: formData
  })
...
const insertItens = (itemNF, itemName, itemQuantity, itemValue, itemTotalValue) => {
  ...
  document.getElementById("itemQtd")
...
const addComments = async (nfOrderTxt, txt) => {
  ...
  document.getElementById(txtBtn).addEventListener("click", addComments())
  if(confirm("Deseja adicionar o comentário:\n" + txt +"\nNf do pedido: "+ order)){
...
// }else{
  // async function commentBtnClick(btn){
    
    // return new Promise(resolve => btn.onclick = () => {
      // return resolve();
    // })
...
for (i = 0; i < dataRow.length; i++) {
  dataRow[i].onclick = async function () {
    ...
    if(confirm("Você selecionou o pedido NF: "+ order +"\nDeseja consultar, cadastrar ou remover novos produtos"))
      console.log('getSelectedOrder', order)
      insertItens(dataRow)
    }
